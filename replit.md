...
-   **Email-First, Multi-Device Architecture**: Students are uniquely identified by `(schoolId, emailLc)` - a composite key that enables true email-first architecture with case-insensitive matching. The `emailLc` column is a generated column created via `lower(trim(email))` with a unique constraint preventing duplicate students per school. This architecture supports:
    ...
    10. **Daily Reset System for Shared Chromebooks**: Student-device associations automatically expire after 24 hours to keep rosters clean in shared Chromebook environments. The system includes: (a) `lastSeenAt` timestamp on each student-device link, updated on every extension registration/heartbeat; (b) `cleanupStaleStudentDevices(hoursOld)` method that deletes associations older than specified threshold; (c) scheduled cleanup job running at midnight (server timezone/UTC) to remove associations older than 24 hours; (d) roster endpoint filtering that excludes stale associations from Device Roster view. **Timezone Limitation**: For single-school pilot deployment, cleanup runs at midnight UTC (server time). Production multi-tenant systems would need per-school timezone configuration.
