...
-   **Email-Based Student Recognition**: Students are identified by email across devices, allowing for automatic student record matching, device ID updates, and prevention of duplicate student records. The system handles two critical duplicate-prevention scenarios:
    1. **Case Sensitivity Fix**: All student emails are normalized (trimmed and lowercased) at the storage layer to ensure case-insensitive matching. This prevents duplicate student creation when CSV imports use mixed-case emails (e.g., "Lucy.Garcia@District.org") and the Chrome Extension Identity API returns lowercase versions (e.g., "lucy.garcia@district.org"). The `normalizeEmail` helper in `shared/utils.ts` handles normalization, and both MemStorage and DatabaseStorage implementations enforce this automatically during create/update operations and lookups.
    2. **Missing Email Reconciliation**: Chrome Extension registrations may arrive without email if the `identity.email` permission is unavailable on managed Chromebooks. The `/api/register-student` endpoint now checks for placeholder devices (pattern: `pending-{email}`) created during CSV import and automatically matches them to the real device instead of creating duplicates. If no placeholder match is found and no email is provided, the endpoint returns a clear error message rather than creating an untrackable student record.
