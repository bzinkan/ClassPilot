...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  // Teacher broadcast start - show presenting overlay
  if (message.type === 'teacher-broadcast-start') {
    showBroadcastOverlay();
  }

  // Teacher broadcast stop - hide presenting overlay
...
function showBroadcastOverlay() {
  ...
  // Remove any existing broadcast overlay
  const existing = document.getElementById('classpilot-broadcast-overlay');
  if (existing) {
    existing.remove();
  }

  const overlay = document.createElement('div');
  overlay.id = 'classpilot-broadcast-overlay';
  overlay.innerHTML = `
    <div class="classpilot-broadcast-banner">
      <div class="classpilot-broadcast-icon">📺</div>
      <div class="classpilot-broadcast-text">
        <span class="classpilot-broadcast-title">Teacher is Presenting</span>
        <span class="classpilot-broadcast-subtitle">Please look at the front of the class</span>
      </div>
      <div class="classpilot-broadcast-pulse"></div>
    </div>
  `;
...
function hideBroadcastOverlay() {
  ...
  const overlay = document.getElementById('classpilot-broadcast-overlay');
  if (overlay) {
    overlay.classList.add('classpilot-broadcast-out');
    setTimeout(() => overlay.remove(), 300);
  }
}

function addBroadcastStyles() {
  if (document.getElementById('classpilot-broadcast-styles')) {
    return;
  }

  const style = document.createElement('style');
  style.id = 'classpilot-broadcast-styles';
  style.textContent = `
    #classpilot-broadcast-overlay {
      position: fixed;
      top: 0;
      left: 0;
      right: 0;
      ...
      padding-top: 20px;
      pointer-events: none;
      animation: classpilot-broadcast-slide-in 0.4s ease-out;
    }

    #classpilot-broadcast-overlay.classpilot-broadcast-out {
      animation: classpilot-broadcast-slide-out 0.3s ease-in forwards;
    }

    @keyframes classpilot-broadcast-slide-in {
      from { transform: translateY(-100px); opacity: 0; }
      to { transform: translateY(0); opacity: 1; }
    }

    @keyframes classpilot-broadcast-slide-out {
      from { transform: translateY(0); opacity: 1; }
      to { transform: translateY(-100px); opacity: 0; }
    }

    .classpilot-broadcast-banner {
      display: flex;
      align-items: center;
      gap: 16px;
      ...
      padding: 16px 28px;
      border-radius: 16px;
      box-shadow: 0 8px 32px rgba(220, 38, 38, 0.4), 0 0 0 1px rgba(255, 255, 255, 0.1) inset;
      font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif;
      pointer-events: auto;
      position: relative;
      overflow: hidden;
    ...
    .classpilot-broadcast-icon {
      font-size: 32px;
      animation: classpilot-broadcast-pulse-icon 1.5s ease-in-out infinite;
    }

    @keyframes classpilot-broadcast-pulse-icon {
      0%, 100% { transform: scale(1); }
      50% { transform: scale(1.15); }
    }

    .classpilot-broadcast-text {
      display: flex;
      flex-direction: column;
      gap: 4px;
    }

    .classpilot-broadcast-title {
      font-size: 18px;
      font-weight: 700;
      letter-spacing: -0.5px;
    }

    .classpilot-broadcast-subtitle {
      font-size: 13px;
      opacity: 0.9;
      font-weight: 500;
    }

    .classpilot-broadcast-pulse {
      position: absolute;
      right: 20px;
      width: 12px;
      height: 12px;
      background: #fef2f2;
      border-radius: 50%;
      animation: classpilot-broadcast-dot-pulse 1.5s ease-in-out infinite;
    }

    .classpilot-broadcast-pulse::before {
      content: '';
      position: absolute;
      top: 50%;
      left: 50%;
      ...
      border-radius: 50%;
      animation: classpilot-broadcast-ring-pulse 1.5s ease-in-out infinite;
    }

    @keyframes classpilot-broadcast-dot-pulse {
      0%, 100% { opacity: 1; }
      50% { opacity: 0.6; }
    }

    @keyframes classpilot-broadcast-ring-pulse {
