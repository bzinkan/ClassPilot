...
>
> The 2.9.1 candidate is being prepared; no packaging, tagging, upload or
> publication is authorized by these instructions. After separate authorization,
> use only `dist/ClassPilot-v2.9.1.zip` produced from the clean,
> tagged, reviewed commit by `./extension/package-extension.sh`. The matching
> `dist/ClassPilot-v2.9.1.zip.sha256` record, commit SHA, CI evidence, and exact
...
An earlier archive is not releasable as 2.9.1.
...
node scripts/verify-extension-package.mjs dist/ClassPilot-v2.9.1.zip --verify-only
```
...
7. Upload the retained `dist/ClassPilot-v2.9.1.zip` whose SHA-256 was verified
...
3. Run the complete gates and `./extension/package-extension.sh` from the
   repository root.
4. Verify and retain `dist/ClassPilot-v2.9.1.zip`, its SHA-256, source/ZIP byte
