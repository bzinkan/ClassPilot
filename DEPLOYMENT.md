...
>
> The 2.9.2 candidate is being prepared; no packaging, tagging, upload or
> publication is authorized by these instructions. After separate authorization,
> use only `dist/ClassPilot-v2.9.2.zip` produced from the clean,
> tagged, reviewed commit by `./extension/package-extension.sh`. The matching
> `dist/ClassPilot-v2.9.2.zip.sha256` record, commit SHA, CI evidence, and exact
> uploaded archive must be retained. Never create or upload a ZIP manually.
> Earlier archives do not contain the 2.9.2 Class tools additions.
...
> submitted for this release. Version 2.9.0 also retains the restricted-sign-in
...
> and `schoolWebsiteBlockEnforcementV1`. Preparing or publishing this extension
> does not enable a school capability or change its monitoring settings. Verify
> each new contract on controlled managed devices before any wider activation;
> see `extension/README.md` and `extension/COMPLIANCE.md` for scope and disclosure.
...
screenshot lanes, exact-authority active-class screenshot cadence, the
...
Preserve 2.8.9's exclusive scheduled supervision authority for student tools,
restored overlays and media contracts, with stale ownership and expiry guards.
Version 2.9.0 adds the startup recovery correction: a completed startup storage
failure recovers on Retry or bounded backoff once storage returns, a storage
operation that never reports back is reconciled by a fresh read, managed-policy
...
An earlier archive is not releasable as 2.9.2.
...
node scripts/verify-extension-package.mjs dist/ClassPilot-v2.9.2.zip --verify-only
```
...
7. Upload the retained `dist/ClassPilot-v2.9.2.zip` whose SHA-256 was verified
...
### 4.5 Test limited safety monitoring and school website policies

Use the exact versioned release ZIP on at least two controlled Google
Admin-managed Chromebooks with the intended school settings. No automated
Chromium suite can attest to the installed enterprise policy or fleet rollout.

1. Confirm the school's timezone, instructional closures, date overrides,
   ordinary window, and an overnight window use the intended school dates.
2. Exercise Off, Safety only, and Full after-hours settings. Safety only must send only
   the current tab's URL/title observation for safety classification, with no
   other-tab snapshot, screenshot, teacher classroom connection, or tab closure
   caused by the safety finding. Without capability agreement, it must stay off.
3. During full monitoring, block a test domain and verify navigation is denied,
   matching existing tabs close, unrelated and lookalike hosts remain open,
   and the school receives the correct policy revision and application result.
   Unblock it and confirm a new navigation is allowed.
4. Restart the MV3 worker, interrupt an acknowledgment, and change the signed-in
   student while an update is pending. Retries must preserve the exact policy
   revision and student authority without applying stale work to the new student.
5. Repeat managed-policy-change and kill-switch checks. Preserve evidence of
   these controlled-device checks separately from automated test results.

### 4.6 Test Screen Sharing
...
3. Run the complete gates and `./extension/package-extension.sh` from the
   repository root.
4. Verify and retain `dist/ClassPilot-v2.9.2.zip`, its SHA-256, source/ZIP byte
