...
describe("2.9.0 release documentation and preserved sign-in guarantees", () => {
  it("pins every active release instruction to the versioned 2.9.0 artifact", () => {
    ...
    for (const source of [readme, compliance, deployment]) {
      expect(source).toContain("ClassPilot-v2.9.0.zip");
    ...
    expect(deployment).toContain("An earlier archive is not releasable as 2.9.0.");
  ...
  it("requires the paired migration and explicit rollout while keeping Live View UI disabled", () => {
    for (const path of ["DEPLOYMENT.md", "extension/README.md", "extension/COMPLIANCE.md", "CLASSPILOT_2_8_9_RELEASE.md", "CLASSPILOT_2_9_0_RELEASE.md"]) {
    ...
    for (const path of ["CLASSPILOT_2_8_9_RELEASE.md", "CLASSPILOT_2_9_0_RELEASE.md"]) {
